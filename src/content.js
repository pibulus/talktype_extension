...
function getTextInputsInShadowDom(shadowRoot) {
  ...
  const standardInputs = shadowRoot.querySelectorAll('input[type="text"], input[type="search"], input:not([type]), textarea');
  ...
  const editableElements = shadowRoot.querySelectorAll('[contenteditable="true"]');
  ...
  const richEditors = shadowRoot.querySelectorAll('.ql-editor, .ProseMirror, .public-DraftEditor-content, [role="textbox"]');
...
function initializeFocusTracking() {
  console.log('TalkType: Initializing enhanced focus tracking for contextual transcription');
...
function isValidTextInputElement(element) {
  ...
  if (element.isContentEditable) {
    // For Gmail and other Google products, be more permissive
    ...
    const isGmailEditor = element.classList.contains('editable') || 
                          element.classList.contains('Am') || 
                          element.classList.contains('Al') ||
                          (element.getAttribute('g_editable') === 'true');
                          
    // If this is a Google product with known editor patterns, be more permissive
    if (isGoogle && (isGmailEditor || 
    ...
    const hasTextClasses = elementClasses.match(/input|text|edit|field|area|compose|comment|editor|rich/) !== null ||
                          elementClasses.includes('ql-editor') ||
                          elementClasses.includes('prosemirror') ||
                          elementClasses.includes('drafts') ||
                          elementClasses.includes('editable');
                          
    const hasPlaceholder = element.getAttribute('placeholder') !== null ||
                          element.getAttribute('data-placeholder') !== null;
    
    // Look for element descendants that suggest it's not a text input
    // But be more permissive about editors that might have formatting controls
    let hasInteractiveDescendants = false;
    
    // Only consider it problematic if the interactive elements aren't typical editor controls
    const interactiveElements = element.querySelectorAll('button, select, [role="button"], [role="menuitem"], [role="tab"]');
    if (interactiveElements.length > 0) {
      // Check if these appear to be formatting controls
      let formattingControls = 0;
      interactiveElements.forEach(el => {
        const elClass = el.className.toLowerCase();
        if (elClass.includes('format') || elClass.includes('bold') || elClass.includes('italic') || 
            elClass.includes('underline') || elClass.includes('link') || elClass.includes('style')) {
          formattingControls++;
        }
      });
      
      // If most of the elements appear to be formatting controls, don't disqualify
      hasInteractiveDescendants = formattingControls < interactiveElements.length / 2;
    }
    
    // Check for explicit aria-label suggesting this is a text field
    const ariaLabel = (element.getAttribute('aria-label') || '').toLowerCase();
    const hasTextLabel = ariaLabel.match(/comment|text|write|input|compose|editor|post|message|reply|search|email|mail|note/) !== null;
    ...
    if (isFacebook && (element.getAttribute('data-lexical-editor') === 'true' || 
                       elementClasses.includes('notranslate'))) {
      return true;
    }
    
    // Gmail and Google Docs specific detection
    if (ariaLabel.includes('message body') || 
        ariaLabel.includes('compose email') || 
        elementClasses.includes('editable')) {
      return true;
    }
  ...
  if (tagName === 'iframe') {
    // More comprehensive iframe detection for editors
    if (element.id?.includes('editor') || 
        element.name?.includes('editor') ||
        element.className?.includes('editor') ||
        element.src?.includes('editor') ||
        element.title?.includes('editor') ||
        element.getAttribute('aria-label')?.includes('editor')) {
      return true;
    }
    
    // Try to detect Google Docs iframe
    if (window.location.hostname.includes('google') && 
        (element.className?.includes('docs') || 
         element.id?.includes('docs') ||
         element.src?.includes('document'))) {
      return true;
    }
    
    // Special case for Gmail compose iframe
    if (window.location.hostname.includes('gmail') &&
        element.getAttribute('aria-label')?.toLowerCase().includes('compose')) {
      return true;
    }
  }
  ...
  // Detect common patterns for text editors across sites
  // This catches many inputs that might be missed by other checks
  ...
  for (let i = 0; i < attrs.length; i++) {
    ...
    // Check for attributes that suggest this is a text input
    if ((name === 'data-gramm' && value === 'true') || // Grammarly integration
        (name === 'data-gramm_editor' && value === 'true') ||
        (name === 'data-slate-editor' && value === 'true') || // Slate.js editor
        (name === 'data-lexical-editor' && value === 'true') || // Lexical editor (Facebook)
        (name.includes('editor') && value === 'true') ||
        (name.includes('contenteditable')) ||
        (name.includes('editable')) ||
        (name.includes('wysiwyg'))) {
      return true;
    }
  }
  
  // Check for Gmail's dynamic content
...
function getDeepActiveElement() {
  ...
  if (window.location.hostname.includes('mail.google.com') || 
    ...
    if (!isValidTextInputElement(active)) {
      const gmailComposer = document.querySelector('div[role="textbox"][aria-label*="compose"], div[g_editable="true"], div.Am.Al.editable[role="textbox"]');
      if (gmailComposer) {
...
function insertIntoShadowDomInput(input, text) {
  ...
  try {
    ...
    if (selection.rangeCount > 0 && (selection.getRangeAt(0).commonAncestorContainer === input ||
...
function insertIntoIframe(iframe, text) {
  try {
    ...
    if (!iframeDoc) {
      console.log('TalkType: Cannot access iframe content (possible cross-origin restriction)');
      return false;
    }
    ...
    if (iframeActive && (iframeActive.isContentEditable || 
...
function insertIntoGoogleDocs(text) {
  ...
  try {
    ...
    if (!document.execCommand('insertText', false, text)) {
      const hiddenTextarea = document.querySelector('.docs-texteventtarget-iframe');
      if (hiddenTextarea) {
...
function getSpecialWebAppInputs() {
  ...
  } else if (host.includes('facebook.com') || host.includes('instagram.com') || host.includes('messenger.com')) {
...
function insertTextIntoField(text, requestId = 'manual') {
  console.log('TalkType: Smart insertion requested (ID:', requestId, '):', text?.substring(0, 20) + '...');
  ...
  if (!insertionHandled && activeInput && isValidAndAccessibleInput(activeInput)) {
    try {
      activeInput.focus();
      ...
      if (activeInput.tagName === 'TEXTAREA' || 
        ...
        // Insert text at cursor position
        activeInput.value = currentValue.substring(0, selStart) + 
                           text + 
  ...
  if (!insertionHandled) {
    ...
    if (fallbackInput && fallbackInput !== activeInput && 
      ...
      console.log('TalkType: Using document.activeElement as fallback (including Shadow DOM)');
      ...
      try {
        if (fallbackInput.tagName === 'TEXTAREA' || 
          ...
          fallbackInput.value = currentValue.substring(0, selStart) + 
                               text + 
                               currentValue.substring(selEnd);
  ...
  if (!insertionHandled) {
    try {
      ...
      console.error('TalkType: Error in last resort insertion:', lastResortError);
    }
  }
  ...
  return {
    ...
    message: insertionHandled ? "Text inserted successfully" : "Could not find valid input for insertion"
  };
}
...
async function stopContextMenuRecording() {
  ...
  try {
    ...
    if (contextMenuRecordingIndicator) {
      const textElement = contextMenuRecordingIndicator.querySelector('.talktype-context-recording-text');
      if (textElement) {
        textElement.textContent = 'Processing transcription...';
      }
      
      // Change style to indicate processing
      contextMenuRecordingIndicator.style.background = 'rgba(70, 174, 247, 0.85)';
    }
    ...
    // Transcribe audio
    const transcription = await freshApiService.transcribeAudio(audioBlob);
    console.log('TalkType: Context menu transcription complete:', transcription);
    ...
    if (targetInputElement && isValidTextInputElement(targetInputElement)) {
      ...
      } else if (targetInputElement.tagName === 'INPUT' || targetInputElement.tagName === 'TEXTAREA') {
        // For standard input/textarea elements
      ...
      showStatusNotification('Could not insert transcription: Input not found', 'error');
    }
  } catch (error) {
