...
function isValidTextInputElement(element) {
  ...
  if (element.isContentEditable) {
    ...
    const hasTextClasses =
      elementClasses.match(
        /input|text|edit|field|area|compose|comment|editor|rich/
      ) !== null ||
    ...
    const interactiveElements = element.querySelectorAll(
      'button, select, [role="button"], [role="menuitem"], [role="tab"]'
    );
    if (interactiveElements.length > 0) {
      ...
      interactiveElements.forEach((el) => {
        ...
        if (
          elClass.includes("format") ||
          elClass.includes("bold") ||
          elClass.includes("italic") ||
          elClass.includes("underline") ||
          elClass.includes("link") ||
          elClass.includes("style")
        ) {
          formattingControls++;
        }
      });
      ...
      hasInteractiveDescendants =
        formattingControls < interactiveElements.length / 2;
    }
    ...
    const hasTextLabel =
      ariaLabel.match(
        /comment|text|write|input|compose|editor|post|message|reply|search|email|mail|note/
      ) !== null;
  ...
  for (let i = 0; i < attrs.length; i++) {
    ...
    if (
      (name === "data-gramm" && value === "true") || // Grammarly integration
