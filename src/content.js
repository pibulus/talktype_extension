...
function initializeFocusTracking() {
  ...
  setInterval(() => {
    ...
    if (!activeInput && deepActive && window.InputDetectionService.isValidTextInputElement(deepActive)) {
      console.log(
...
function initializeInputDetection() {
  ...
  clearTextInputs.forEach((element) => {
    if (!element.dataset.hasMicButton && window.InputDetectionService.isValidTextInputElement(element)) {
...
// Note: The isValidTextInputElement function has been removed in favor of using 
// window.InputDetectionService.isValidTextInputElement() directly
    ...
    if (activeInput === event.target) {
      ...
      setTimeout(() => {
        ...
        if (
          ...
          !window.InputDetectionService.isValidTextInputElement(document.activeElement)
        ) {
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  ...
  if (request.action === "startTranscriptionFromContextMenu") {
    ...
    if (!window.InputDetectionService.isValidTextInputElement(targetInputElement)) {
      console.error(
...
async function stopContextMenuRecording() {
  ...
  try {
    ...
    if (targetInputElement && window.InputDetectionService.isValidTextInputElement(targetInputElement)) {
